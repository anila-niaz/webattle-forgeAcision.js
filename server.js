            ...
            switch (msg.t) {
                case Serializer.MSG_NEW_PLAYER:
                    ...
                    for (var i = 0; i < players.length; i++) {
                        if(players[i].sid == client.sessionId)
                            players[i].addName(name);
                    }
                    var playerNames = getPlayerNames();

                    var message = Serializer.serialize(Serializer.MSG_PLAYERS, {players: playerNames});
...
var getPlayerNames = function(){
    return players.filter(hasName)
        .map(toName);

    function hasName(player){
        return player.name != undefined && player.name.length > 0;
    }
